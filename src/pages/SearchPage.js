...
function SearchPage(){
    //get the search term, which is inside the state [state, dispatch]
...
//{term && - ^^only render the search result if you have provided the search term. if term = true}


//to get the required image link code:
/*
{item.pagemap?.cse_image?.length>0 && item.pagemap?.cse_image[0]?.src && (
    <img className = "searchPage__resultImage"
        src={item.pagemap?.cse_image[0]?.src}
